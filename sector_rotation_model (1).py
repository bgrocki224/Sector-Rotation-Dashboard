...
class PortfolioConstructor:
    ...
    def target_weights(self, scores: pd.Series, current_holdings: set[str]) -> dict[str, float]:
        ...
        # apply the max-weight cap
        capped = raw_w.clip(upper=self.cfg["max_weight"])

        # apply the min-weight floor for inclusion -- but never let it drop the
        # portfolio below the min_sectors floor. If dropping sub-floor names would
        # breach the minimum, keep them all rather than violate the sector-count rule.
        above_floor = capped[capped >= self.cfg["min_weight"]]
        if len(above_floor) >= min_sectors:
            capped = above_floor

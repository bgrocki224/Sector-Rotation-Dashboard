...
- **Lookahead bias:** fundamentals/analyst data only exist as today's snapshot (no free
  ...
  base `DataProvider` (momentum/trend/seasonality/calendar only); only the live "current
...
**On the `calendar` factor:** it's deliberately identical across all sectors (it's a
market-wide flag — turn-of-month, presidential cycle — not sector-specific), so it has
zero effect on the actual ranking once z-scored, even though it shows a non-zero number
in the raw factor table. Per-sector calendar effects are what the `seasonality` factor
already covers.

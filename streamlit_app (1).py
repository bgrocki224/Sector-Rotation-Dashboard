...
def run_backtest(start_date: str, cfg: dict):
    ...
    # are neutral (0) in this backtest; only momentum/trend/seasonality/calendar
...
use_extended = st.sidebar.toggle(
    ...
    help="When on, fundamentals/analyst_sentiment/market_sentiment are computed from "
         ...
         "the ranking runs on momentum/trend/seasonality/calendar only. Turn off if "
...
cfg["top_n"] = st.sidebar.slider("Top N sectors held", 2, 8, CONFIG["top_n"])
...
st.caption(
    "'calendar' is intentionally identical across all sectors (it's a market-wide flag, "
    "not sector-specific) — because it has zero variance across sectors, it contributes "
    "nothing to the ranking below once z-scored, even though the raw number is non-zero here."
)
...
st.caption(
    "Backtest always runs on momentum/trend/seasonality/calendar only, regardless of the "
